...
const AlignCenterModal = ({ isOpen, onClose, onSubmit, person }) => {
  return (
    <Modal isOpen={isOpen} onClose={onClose} align="center" rounded="md" bg="danger400">
      <Icon
        name="Cross"
        pos="absolute"
        top="1rem"
      ...
      />
      <Div d="flex" m={{ b: '4rem' }}>
        <Icon name="AlertSolid" color="danger700" m={{ t: '0.35rem', r: '0.5rem' }} />
...
const DeleteModal = ({ id, setPersons, persons, person }) => {
  ...
  return (
    <>
      <Button bg="danger700" hoverBg="danger600" m={{ r: '0.5rem', y: '0.5rem' }} onClick={Handleclick}>
        <Icon name="Delete" size="20px" />
      </Button>
      <AlignCenterModal isOpen={showModal} onClose={Handleclick} onSubmit={Handlesubmit} person={person} />
    </>
  );
};
